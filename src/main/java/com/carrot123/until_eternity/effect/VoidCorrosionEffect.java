...
public final class VoidCorrosionEffect extends MobEffect {
    ...
    public void applyEffectTick(LivingEntity target, int amplifier) {
        ...
        float amount = VoidCorrosionDamageLogic.periodicDamage(
                target.getMaxHealth(), target.getHealth());
        if (amount <= 0.0F) {
            return;
        }
        DamageSource source = target.damageSources().fellOutOfWorld();
        VoidCorrosionDamageContext.hurt(target, source, amount);
    }
}
