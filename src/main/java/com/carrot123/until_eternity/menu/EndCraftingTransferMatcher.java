...
public final class EndCraftingTransferMatcher {
    ...
    public static Match match(
        ...
        for (int group = 0; group < groupCounts.length; group++) {
            if (sets > groupStackLimits[group]) {
                continue;
            }

            int capacity = Math.min(
                    cellCount,
                    groupCounts[group] / sets
            );
        ...
        for (int cell = 0; cell < cellCount; cell++) {
            ...
            for (int unit = 0; unit < unitGroups.size(); unit++) {
                int group = unitGroups.get(unit);

                if (acceptedGroups[cell][group]) {
                    cellCandidates.add(unit);
                }
            }
            ...
            cellCandidates.sort(
                    Comparator
                            .comparingInt(
                                    (Integer unit) ->
                                            unitGroups.get(unit) == currentGroup ? 0 : 1
                            )
        ...
        for (int cell = 0; cell < assignments.length; cell++) {
